...
<script>
	...
	function verarchivo() {
		...
		$.ajax({
			type:"post",
			url : "excel/xecompensaciones.php",
			data: { 
				anio: $("#anio").val()
			},
			success : function (data) {
				...
				t=12;
				c = (datos.length-1)/t;
				tabla = "<table class='table table-bordered table-condensed'><tr><th>No.</th><th>MES</th><th>DO</th><th>SAP DOC No.</th><th>BL</th><th>FECHA BL</th><th>CLIENTE</th><th>No. FACTURA</th><th>INCOTERM</th><th>VALOR FOB USD</th><th>VALOR USD</th><th>DEX</th><th>FECHA</th></tr>";
				...
				for(f=1; f<c ;f++) {										
					if($("#mees").val()!="Todos") {
						if(datos[f*t]==$("#mees").val()) {
							...
							tabla += "<tr "+colorb+"><td>"+f+"</td><td>"+datos[f*t]+"</td><td>"+datos[f*t+1]+"</td><td>"+datos[f*t+2]+"</td><td>"+datos[f*t+3]+"</td><td>"+datos[f*t+4]+"</td><td>"+datos[f*t+5]+"</td><td>"+datos[f*t+6]+"</td><td>"+datos[f*t+7]+"</td><td>"+datos[f*t+8]+"</td><td>"+datos[f*t+9]+"</td><td>"+datos[f*t+10]+"</td><td>"+datos[f*t+11]+"</td></tr>";
						}
					} else {
						if(f==0) colorb = "style='background-color:#D0F7D5'";
						 	else colorb = "";
