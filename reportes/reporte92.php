<?php
    ...
    // Titulo del reporte
    $titulo="FACTURAS DAIMLER";
    
    // Nombre del archivo 
    $archivo="facturas_daimler";
    ...
    $dimensionCol = array (51.43,  26.57,  26.57,  20.71,  46.43,  18.43,  10.71,  10.71);
    ...
    $sql  = "SELECT 'AGENCIA DE ADUANAS HUBEMAR SAS NIVEL 1' PROVEEDOR,
        CONCAT(P.MASCARA, '-', ED.NUMERO_FACTURA) 'FACTURA', 
        ED.NUMERO_PEDIDO 'NUMERO PEDIDO',
        MT.NOMBRE_MEDIO_TRANS 'NOMBRE MEDIO TRANS',  
        IF(ED.TIPO_REGIMEN='I', EX.NOMBRE_EXPORTADOR, EXDTA.NOMBRE_EXPORTADOR) 'EXPORTADOR',
        CF.FECHA_FACTURA 'FECHA FACTURA',
        CF.NUMERO_FACTURA 'NUMERO FACTURA', 
        IF(ED.TIPO_REGIMEN='I', 'ZINA', IF(ED.TIPO_REGIMEN='T', 'ZDTA','INVALIDO')) 'CONDICIÓN'
        ...
        LEFT OUTER JOIN dtas_forms                      DTA   ON DTA.NUMERO_DO            = DT.NUMERO_DO
        LEFT OUTER JOIN EXPORTADORES                    EXDTA ON EXDTA.ID_EXPORTADOR=DTA.ID_EXPORTADOR
        LEFT OUTER JOIN IMPORTACIONES                   IM    ON IM.NUMERO_DO=ED.NUMERO_DO
        LEFT OUTER JOIN tipos_docs_soporte_sucursales   P     ON P.CODIGO_SUCURSAL=ED.CODIGO_SUCURSAL_OP 
            AND P.ID_TIPO_DOC_SOPORTE='2'
        ...
        if(!empty($res)) {
            ...
            for($j=0; $j<$nf; $j++) {
               ...
               if($tipo=="XLS"){
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('A3:' .$columnas[$nf-1] . '3' )->applyFromArray($estiloTituloColumnas);
               }
            }
            ...
            if($tipo=="XLS"){
                ...
                $objPHPExcel->getActiveSheet()->setAutoFilter("A3:" . $columnas[$j-1] . "3");

                // Inmovilizar paneles
                $objPHPExcel->getActiveSheet()->getStyle( $columnas[$j] . '3' )->applyFromArray($estiloInformacion);
