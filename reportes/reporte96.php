<?php
    ...
    $param_fecha="";
    // Titulo del reporte
    $titulo="PROGRAMA DE CARGUE";
    // Nombre del archivo 
    $archivo="programa_cargue";
    $registros = "";
    ...
    $dimensionCol = array (14.29,  20.29,  20.29,  8.57,  8.57,  20.29,  35.43,  42.71,  12.43,  12.43,  20.29,  12.43,  20.29,  12.43,  20.29,  12.43,  20.29,  54.43);


    $tipodato= array('T','T','T','T','N','T','T','T','D','D','D','D','D','D','T','D','T','T');

    $calendario_festivo=festivos();

    if($numerodo=="" && $numeroped=="" && $ordencompra=="" && $doctrans=="" ){

        $filtros = " WHERE ED.ANULADO='N' AND  EX.FECHA_ZARPE IS NULL AND ED.TIPO_REGIMEN='E'  AND  DT.FECHA_RADICACION_PUERTO IS NULL  ";
    ...
    $sql  = "SELECT ED.NUMERO_DO 'DO', 
        ...
        UPPER(DP.NOMBRE_DEPOSITO) 'LUGAR EMBARQUE', 
        DATE_FORMAT(VM.FECHA_ETA, '%d/%m/%Y') 'FECHA ETA', 
        DATE_FORMAT(VM.CUT_OFF_FISICO, '%d/%m/%Y')  'CUT OFF FISICO',
        DATE_FORMAT(VM.CUT_OFF_FISICO, '%H:%i')  'HORA CUT OFF FISICO',
        DATE_FORMAT(VM.CUT_OFF_DOCUMENTAL, '%d/%m/%Y')  'CUT OFF DOCUMENTAL',
        DATE_FORMAT(VM.CUT_OFF_DOCUMENTAL, '%H:%i')  'HORA CUT OFF DOCUMENTAL',
        DATE_FORMAT(DT.FECHA_ENVIO_ASIGNACION_TRANS, '%d/%m/%Y') 'FECHA ASIGNACION TRANSPORTE',
        DT.BOOKING_NUMBER 'BOOKING NUMBER', 
        DATE_FORMAT(EX.FECHA_INGRESO_SUGERIDO, '%d/%m/%Y') 'FECHA INGRESO SUGERIDO', 
        GROUP_CONCAT(DISTINCT SA.ID_DOCUMENTO_MUISCA) 'NUM AUT EMBARQUE'

    
      FROM EXPORTACIONES                 EX

      INNER JOIN ESTADOS_DO                         ED ON EX.NUMERO_DO=ED.NUMERO_DO
      LEFT OUTER JOIN ex_solicitudes_aut_embarques  SA ON SA.NUMERO_DO=ED.NUMERO_DO
