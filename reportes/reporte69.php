<?php
    ...
    $param_fecha="";
    // Titulo del reporte
    $titulo="INFORME EXPORTACIONES";
    // Nombre del archivo 
    $archivo="InformeExportaciones";
    $registros = "";
    ...
    $dimensionCol = array (16, 16, 26, 12.50, 12.50, 12.50, 40, 12.50, 12.50, 12.50, 30, 16.14, 12.50, 19.14, 18.14, 13);

    $tipodato= array('T','T','T','T','T','D','T','N','T','T','T','D','D','T','T','D');
    ...
    if($numerodo=="" && $numeroped=="" && $ordencompra=="" && $doctrans=="" ){
        ...
        if($flevantei<>"") $param_fecha .="AND (E.FECHA_DEX >= '$flevantei 00:00:00'   
                                           AND  E.FECHA_DEX <= '$flevantef 23:59:59') ";
        if($faceptai<>"")  $param_fecha .="AND (E.FECHA_AUT_EMBARQUE >= '$faceptai 00:00:00'   
                                           AND  E.FECHA_AUT_EMBARQUE <= '$faceptaf 23:59:59') ";
        if($fmercanciai<>"") $param_fecha .="AND (VM.FECHA_ETA >= '$fmercanciai 00:00:00' AND  VM.FECHA_ETA  <= '$fmercanciaf 23:59:59') ";
        if($fretiroi<>"")  $param_fecha .="AND (DT.FECHA_CARGUE_EN_PLANTA >= '$fretiroi 00:00:00'    
                                           AND  DT.FECHA_CARGUE_EN_PLANTA  <= '$fretirof 23:59:59') ";
        if($fstickeri<>"") $param_fecha .="AND (E.FECHA_ZARPE >= '$fstickeri 00:00:00'   
        AND  Ex.FECHA_ZARPE        <= '$fstickerf 23:59:59') ";
    ...
    $sql  = "SELECT ED.NUMERO_DO 'NUMERO DO',
        ED.NUMERO_PEDIDO 'NÚMERO PEDIDO',
        I.NOMBRE_IMPORTADOR 'IMPORTADOR',
        DT.CODIGO_CONDICION_ENTREGA 'CONDICIÓN ENTREGA',
        CF.NUMERO_FACTURA 'NÚMERO FACTURA',
        DATE_FORMAT(CF.FECHA_FACTURA, '%d/%m/%Y') 'FECHA FACTURA',
        EX.NOMBRE_EXPORTADOR 'CONSIGNATARIO',
        CF.TOTAL_FACTURA 'TOTAL FACTURA',
        CF.FLETE_US 'FLETE US', 
        CF.SEGURO_US 'SEGURO US',
        DT.NUMERO_DOCUMENTO_TRANSPORTE 'NÚMERO DOCUMENTO TRANSPORTE', 
        DATE_FORMAT(DT.FECHA_DOCUMENTO_TRANSPORTE, '%d/%m/%Y') 'FECHA DOCUMENTO TRANSPORTE',
        DATE_FORMAT(E.FECHA_DEX, '%d/%m/%Y') 'FECHA DEX',
        GROUP_CONCAT(SAU.NUMERO_DEX) 'NÚMERO DEX', 
        GROUP_CONCAT(SAU.ID_DOCUMENTO_MUISCA) 'ID DOCUMENTO MUISCA',
        DATE_FORMAT(GROUP_CONCAT(E.FECHA_ZARPE), '%d/%m/%Y') 'FECHA ZARPE'
        ...
        LEFT OUTER JOIN VIAJES_MOTONAVES               VM   ON VM.ID_VIAJE_MOTONAVE=DT.ID_VIAJE_MOTONAVE
          ...
          GROUP BY ED.NUMERO_DO, CF.NUMERO_FACTURA
    ";      
