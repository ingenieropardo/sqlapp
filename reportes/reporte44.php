<?php
    ...
    $dimensionCol = array (18, 50, 12, 12, 15, 15, 15, 16, 22, 11, 70, 21, 17, 30, 18, 18, 16);

    $tipodato = array ("T","T","N","N","N","N","N","N","T","T","T","T","T","T","T","T","D");
    ...
    $sql  = "SELECT  A.NOMBRE_ADMINISTRACION CIUDAD, 
        EX.NOMBRE_EXPORTADOR 'EXPORTADOR/PROVEEDOR', 
        CAST(FD.PORCENTAJE_ARANCEL AS  DECIMAL(6,2)) '% ARANCEL', 
        CAST(FD.PORCENTAJE_IVA AS  DECIMAL(6,2)) '% IVA',                
        ROUND((( FD.TOTAL_ARANCEL/ FD.VALOR_TOTAL_FOB)  * (DF.EXW_PRORRATEO+DF.SUM_NETO_PRORRATEO+DF.VALOR_FOB_PRORRATEO) ),-3) 'ARANCEL $',
        ROUND((( FD.TOTAL_IVA/ FD.VALOR_TOTAL_FOB) * (DF.EXW_PRORRATEO+DF.SUM_NETO_PRORRATEO+DF.VALOR_FOB_PRORRATEO) ),-3) 'IVA $',
        ROUND((( FD.TOTAL_RESCATE/ FD.VALOR_TOTAL_FOB) * (DF.EXW_PRORRATEO+DF.SUM_NETO_PRORRATEO+DF.VALOR_FOB_PRORRATEO) ),-3) 'RESCATE $',
        ROUND((( FD.TOTAL_LIQUIDADO/ FD.VALOR_TOTAL_FOB) * (DF.EXW_PRORRATEO+DF.SUM_NETO_PRORRATEO+DF.VALOR_FOB_PRORRATEO) ),-3) ' TOTAL TRIBUTOS $',
        DF.ORDEN_COMPRA  'No. OC', 
        ' ' ITEM, 
        DF.NOMBRE_COMERCIAL  'PRODUCTO',
        CF.NUMERO_FACTURA  'No. FACTURA IMPORTACION', 
        FD.CODIGO_POSICION   'PARTIDA ARANCELARIA', 
        CONVERT(P.CODIGO_PRODUCTO, CHAR)  'CODIGO SAP',  
        FD.NUMERO_ACEPTACION  'NUMERO ACEPTACION', 
        FD.NUMERO_LEVANTE 'NUMERO LEVANTE', 
        DATE_FORMAT(FD.FECHA_LEVANTE, '%d/%m/%Y') 'FECHA DE LEVANTE'
                    
        FROM FORMULARIOS_DIS FD

        INNER JOIN  ESTADOS_DO           ED     ON ED.NUMERO_DO = FD.NUMERO_DO
        INNER JOIN  DETALLES_FACTURAS    DF     ON DF.ID_FORMULARIO_DI = FD.ID_FORMULARIO_DI
        ...
        INNER JOIN  IMPORTADORES         IM     ON ED.ID_IMPORTADOR = IM.ID_IMPORTADOR
        INNER JOIN  IMPORTACIONES        I      ON I.NUMERO_DO=ED.NUMERO_DO
        INNER JOIN  ADMINISTRACIONES     A      ON A.CODIGO_ADMINISTRACION=I.CODIGO_ADMINISTRACION

        $filtros  
        $param_fecha

        ORDER BY ED.NUMERO_DO, FD.NUMERO_ACEPTACION
    ";    
