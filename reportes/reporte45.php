<?php
    ...
    $param_fecha="";
    // Titulo del reporte
    $titulo="CONSOLIDADO DE DO";
    // Nombre del archivo 
    $archivo="Consolidado_Dos";
    $registros = "";
    ...
    $dimensionCol = array (49, 43, 13, 11, 14, 11, 11, 11.14, 15, 145, 15, 15, 14);

    $tipodato = array ("T","T","T","D","D","T","T","T","T","N","N","N","T");
    ...
    $sql  = "SELECT I.NOMBRE_IMPORTADOR 'NOMBRE IMPORTADOR',
        EM.NOMBRE_PERSONA 'NOMBRE PERSONA',
        ED.CODIGO_SUCURSAL_OP 'CODIGO SUCURSAL OP', 
        DATE_FORMAT(IM.FECHA_LEVANTE, '%Y') 'AÑO',
        ...
        SUM( IF(FD.FECHA_LEVANTE IS NOT NULL,1,0)) 'DOS LEVANTES',
        SUM( IF(LEFT(FD.CODIGO_MODALIDAD,1)='C',1,0)) 'DOS ORDINARIOS',
        SUM( IF(LEFT(FD.CODIGO_MODALIDAD,1)='S',1,0)) 'DOS TEMPORALES',
        SUM( IF(FD.CODIGO_MODALIDAD='C3',2,0)) 'DOS NACIONALIZACIONES',
        ...
        LEFT OUTER JOIN (SELECT NUMERO_DO, FECHA_LEVANTE, FECHA_ACEPTACION, LEFT(CODIGO_MODALIDAD,2)
        CODIGO_MODALIDAD, SUM(TOTAL_ARANCEL) TOTAL_ARANCEL, SUM(TOTAL_IVA) TOTAL_IVA, SUM(VALOR_ADUANA) VALOR_ADUANA, COUNT(NUMERO_ACEPTACION) DECLARACIONES FROM FORMULARIOS_DIS GROUP BY 1 ) FD ON FD.NUMERO_DO=ED.NUMERO_DO
