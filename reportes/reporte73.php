<?php
    ...
    $dimensionCol = array (12.57, 14.29, 10.71, 10.71, 36.57, 10.71, 17.29, 59.29, 18.29, 14.71,
                          13.57, 18.57, 20, 14.57, 20, 14.14, 10.71, 20, 14.29, 14.71, 14.71, 14.71);

    $tipodato = array ('N','T','T','T','T','T','T','T','T','N','T','T','T','D','T','D','T','T','D','N','N','N');
    ...
    $sql  = "SELECT  CONCAT(FD.NUMERO_FORMULARIO_DI, '/', FD1.FORMULARIOS)  CANTIDAD, 
                    ...
                    NULL 'FACTURA NO.', 
                    FD.CODIGO_POSICION 'POSICION ARANCELARIA', 
                    ...
                    TD.NOMBRE_TIPO_DECLARACION 'TIPO DECLARACION', 
                    FD.NUMERO_ACEPTACION 'No. ACEPTACION', 
                    DATE_FORMAT(FD.FECHA_ACEPTACION, '%d/%m/%Y')  'FECHA STICKER', 
                    FD.NUMERO_STICKER 'STICKER No.', 
                    DATE_FORMAT(FD.FECHA_PAGO, '%d/%m/%Y')  'FECHA PRESENTADA BANCO', 
                    NULL 'BANCO PRESENTADO', 
                    FD.NUMERO_LEVANTE 'LEVANTE No.', 
                    DATE_FORMAT(FD.FECHA_LEVANTE, '%d/%m/%Y') 'FECHA LEVANTE', 
                    FD.TOTAL_ARANCEL 'ARANCEL', 
                    FD.TOTAL_IVA 'IVA', 
                    FD.TOTAL_LIQUIDADO 'VALOR IMPUESTOS'

            FROM FORMULARIOS_DIS FD
                INNER JOIN TIPOS_DECLARACION            TD ON TD.CODIGO_TIPO_DECLARACION=FD.CODIGO_TIPO_DECLARACION
                INNER JOIN ESTADOS_DO                   ED ON ED.NUMERO_DO = FD.NUMERO_DO
                INNER JOIN (SELECT NUMERO_DO, COUNT(NUMERO_DO) FORMULARIOS FROM FORMULARIOS_DIS  GROUP BY 1) FD1 ON ED.NUMERO_DO = FD1.NUMERO_DO
        ...
        GROUP BY ED.NUMERO_DO, FD.NUMERO_ACEPTACION
    ";    
    ...
    if($tipo=="XLS" || $tipo=="TXT" || $tipo=="HTML") {
        if(!empty($res)) {
            ...
            while($reg = @mysql_fetch_array($res)) {
                ...
                // Fijar ancho de filas
                if($tipo=="XLS"){
                    $objPHPExcel->getActiveSheet()->getRowDimension($altofilas)->setRowHeight(-1); 
                }
