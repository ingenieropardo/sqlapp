<?php
    ...
    $sql  = "SELECT I.NOMBRE_IMPORTADOR 'NOMBRE IMPORTADOR', ED.NUMERO_DO 'NUMERO DO', ED.ID_MODALIDAD 'ID MODALIDAD',MR.NOMBRE_MODALIDAD 'NOMBRE MODALIDAD', ED.CODIGO_SUCURSAL_OP 'CODIGO SUCURSAL OP', EM.NOMBRE_PERSONA 'NOMBRE PERSONA',ETD.NOMBRE_ETAPA 'NOMBRE ETAPA', DATE_FORMAT(ED.FECHA_APERTURA,'%d/%m/%Y') 'APERTURA', DATE_FORMAT(IM.FECHA_LLEGADA_MCIA,'%d/%m/%Y') 'LLEGADA', DATE_FORMAT(IM.FECHA_DOCS_OK,'%d/%m/%Y') 'FECHA DOCK', NULL, NULL, DATE_FORMAT(IM.FECHA_MANIFIESTO,'%d/%m/%Y') 'TDEPOSITO', NULL, NULL, DATE_FORMAT(IM.FECHA_ACEPTACION,'%d/%m/%Y') 'ACEPTACIÓN', DATE_FORMAT(IM.FECHA_ENVIADO_FACTURAR,'%d/%m/%Y') 'ENVIADO', DATE_FORMAT(ED.FECHA_FACTURA,'%d/%m/%Y') 'FECHA FACTURA', EV.FECHA, LEFT(TRIM(ED.ULTIMO_COMENTARIO_CLIENTE),4700) COMENTARIO, ' ' BLANCO
    ...
    LEFT OUTER JOIN (SELECT numero_do, max(fecha_registro) fecha from eventos_do ORDER BY FECHA_REGISTRO DESC ) EV ON EV.NUMERO_DO=ED.NUMERO_DO
