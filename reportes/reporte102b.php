<?php
    ...
    // CLIENTES - ARCHIVOS PLANOS MERK [PDDEPT00]
    $sql = "SELECT FD.NUMERO_STICKER STICKER, P.CODIGO_PRODUCTO CODIGO_REFER_INT_NORMAL, M.CANTIDAD, M.ORDEN_COMPRA, M.VALOR_UNITARIO PRECIO 
            FROM FORMULARIOS_DIS FD 
            LEFT OUTER JOIN DETALLES_FACTURAS M  ON M.ID_FORMULARIO_DI=FD.ID_FORMULARIO_DI 
    ...
    if($tipo=="XLS" || $tipo=="TXT" || $tipo=="HTML") {
        if(!empty($res)) {
            ...
            $titulo = "Clientes - Archivos Planos Merck [PDDEPT00]";
    ...
    switch ($tipo) {            
        case 'XLS':
            ...
            header("Content-Disposition: attachment; filename=PDDEPT00.xls");
        ...
        case 'TXT':
            ...
            header('Content-Disposition: inline; filename=PDDEPT00.txt'); 
        ...
        case 'PDF':
            ...
            $grosor = 6*$nr+60;
            $pdf = new FPDF('L', 'mm', array(140,$grosor));
            ...
            $pdf->Cell(15); $pdf->Cell(0,0,'CLIENTES - ARCHIVOS PLANOS MERK [PDDEPT00]',0,1,'');
            $pdf->Ln(0);
