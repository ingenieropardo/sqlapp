<?php
    ...
    $titulo = "REPORTE APERTURA DE D.O";
    $file="aperturado";
    $registros="";
    ...
    $filtros=" WHERE ED.ANULADO='N' ";
    if($importador!="0") $filtros .= " AND ED.ID_IMPORTADOR='$importador' ";
    if($sucursal  !="0") $filtros .= " AND ED.CODIGO_SUCURSAL_OP='$sucursal' ";
    if($modalidad  !="0" and $modalidad  !=""  ) $filtros .= " AND ED.TIPO_REGIMEN='$modalidad' ";

    include("../config/funciones.php");
    conectaremoto();

    // 
    $sql = "SELECT ED.NUMERO_DO, ED.NUMERO_PEDIDO, IM.NUMERO_IDENTIFICACION, IM.NOMBRE_IMPORTADOR, 
                     S.NOMBRE_SUCURSAL ,DATE_FORMAT( ED.FECHA_APERTURA,'%Y') ANIO,

                    CASE DATE_FORMAT(ED.FECHA_APERTURA, '%m') WHEN '01' Then 'Enero'  WHEN '02' Then 'Febrero'
                         WHEN '03' Then 'Marzo' WHEN '04' Then 'Abril' WHEN '05' Then 'Mayo' WHEN '06' 
                         Then 'Junio'  WHEN '07' Then 'Julio' WHEN '08' Then 'Agosto' 
                         WHEN '09' Then 'Septiembre' WHEN '10' Then 'Octubre' WHEN '11' Then 'Noviembre' 
                         WHEN '12' Then 'Diciembre' END AS MES,  
    
                    ED.TIPO_REGIMEN, ED.TOTAL_FOB, ED.TOTAL_OPERACION_ACTUAL, ED.TRM, T.NOMBRE_TERCERO, 
                    IM.CODIGO_UAP, ED.NUMERO_DOCUMENTO_TRANSPORTE, ED.CERRADO_OPERATIVAMENTE, 
                    ED.CERRADO, ED.NUMERO_FACTURA 
            FROM ESTADOS_DO ED 
                    LEFT OUTER JOIN IMPORTADORES IM ON ED.ID_IMPORTADOR = IM.ID_IMPORTADOR
                    LEFT OUTER JOIN SUCURSALES   S ON S.CODIGO_SUCURSAL =ED.CODIGO_SUCURSAL_OP
                    LEFT OUTER JOIN TERCEROS T ON T.ID_TERCERO=ED.ID_TERCERO 
