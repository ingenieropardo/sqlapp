<?php
    ...
    $param_fecha="";
    // Titulo del reporte
    $titulo="MERCANCIA EMBARCADA";
    // Nombre del archivo 
    $archivo="Mercancia_Embarcada";
    $registros = "";
    ...
    $dimensionCol = array (12, 14.29, 17.29, 17.29, 19.43, 15.43, 13.57, 52, 52, 30.86, 16.86, 16.86, 27, 16, 10.71, 18.57, 10.71, 38.71, 38.71, 38.71, 18.57, 18.57, 31.14, 14.14, 25.71, 30, 15.57, 15.57, 15.57, 15.57, 15.57, 10.71, 18.14, 20.57, 23.14, 23.14, 35);


    $tipodato= array('D','T','T','T','T','D','T','T','T','T','N','N','T','D','N','T','N','T','T','T','D','D','T','T','T','D','D','D','N','D','T','N','T','T','T','T','T','T');
    ...
    $sql  = "SELECT DATE_FORMAT(VM.FECHA_ETS, '%d/%m/%Y') 'FECHA ETS',
        ED.NUMERO_DO 'NUMERO DO', 
        ED.NUMERO_PEDIDO 'NUMERO PEDIDO', 

        CASE UC.CARGA_EN_CONTENEDOR WHEN '1' THEN 'CARGA SUELTA' WHEN '2' THEN 'CONTENEDORIZADA'   WHEN '3' THEN  'GRANEL' WHEN '4' THEN 'MIXTA' WHEN '5' THEN  'ISOTANQUE'  when '6' THEN 'FLEXITANQUE' ELSE 'CARGA SUELTA' END AS 'TIPO CARGA',


        GROUP_CONCAT(DISTINCT CONCAT(FE.NUMERO_FACTURA, ' ')) FACTURA,
        DATE_FORMAT(FE.FECHA_FACTURA, '%d/%m/%Y') 'FECHA FACTURA',  
        UCASE(TRIM(LEFT(ED.DESCRIPCION, 2000))) 'PRODUCTO',
        UCASE(I.NOMBRE_EXPORTADOR) 'IMPORTADOR', 
        UPPER(C.NOMBRE_EXPORTADOR) 'CONSIGNATARIO', 
        UCASE( LI2.NOMBRE_LUGAR_EMBAR_ARRIB_INAL) 'PTO DESTINO',
        DT.PESO_BRUTO 'PESO BRUTO', 
        DT.PESO_NETO 'PESO NETO', 
        EX.NUMERO_DEX 'No DECLARACION', 
        DATE_FORMAT(DT.FECHA_MANIFIESTO_CARGA, '%d/%m/%Y') 'FECHA MANIFIESTO', 
        DATEDIFF(VM.FECHA_ETS, DT.FECHA_MANIFIESTO_CARGA) KPI,  
        ...
        UPPER(TR.NOMBRE_TRANSPORTADOR) 'AGENTE CONSOLIDADOR', 
        NULL 'AGENTE CARGA', 
        DATE_FORMAT(VM.CUT_OFF_FISICO, '%d/%m/%Y') 'CUTOFF', 
        DATE_FORMAT(VM.CUT_OFF_DOCUMENTAL, '%d/%m/%Y') 'CUTOFFFIS', 
        UCASE(VM.NOMBRE_VAPOR)  'MOTONAVE',
        VM.VIAJE_LINEA 'VIAJE', 
        DT.BOOKING_NUMBER 'BOOKING', 
        DT.NUMERO_DOCUMENTO_TRANSPORTE 'BL', 
        DATE_FORMAT(DT.FECHA_DOCUMENTO_TRANSPORTE, '%d/%m/%Y') 'FECHA BL', 
        DATE_FORMAT(EX.FECHA_DOCS_OK, '%d/%m/%Y') 'FECENDOCEX',
        DATE_FORMAT(MD.FECHA_ETA, '%d/%m/%Y') 'FECHA ESTIMADA DE LLEGADA A DESTINO', 
        DATEDIFF(MD.FECHA_ETA, VM.FECHA_ETS ) 'TIEMPO DE TRANSITO OFRECIDO', 
        DATE_FORMAT(MD.FECHA_RTAD, '%d/%m/%Y') 'FECHA DE LLEGADA A DESTINO',  
        DT.CODIGO_CONDICION_ENTREGA 'TERM NEGOC', 
        DT.VALOR_FOB_USD 'FOB', 
        UCASE(PA.NOMBRE_PAIS) 'PAIS DESTINO', 
        UCASE(DT.LUGAR_RECIBO) 'CIUDAD DESTINO',
        UCASE(LE.NOMBRE_LUGAR_EMBAR_ARRIB_NAL) 'LOCALIZACIÓN', 
        UCASE(DP.NOMBRE_DEPOSITO) 'LUGAR EMBARQUE'
    ...
    LEFT OUTER JOIN facturas_exportaciones         FE   ON ED.NUMERO_DO =FE.NUMERO_DO AND FE.TIPO_FACTURA='F'
