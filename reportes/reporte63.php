<?php
    ...
    // Titulo del reporte
    $titulo="ARCHIVO OFICIAL";
    
    // Nombre del archivo 
    $archivo="archivoofocial";
    ...
    $dimensionCol = array (8.86,  20.86,  11.86,  57.71,  29.15,  20.29);
    ...
    $sql  = "SELECT  @curRow := @curRow + 1 AS '#', 
        ED.NUMERO_DO 'NUMERO DO', FD.CANTIDAD 'CANTIDAD', 
      ...
      INNER JOIN (SELECT NUMERO_DO, COUNT(NUMERO_ACEPTACION) CANTIDAD, NUMERO_ACEPTACION, FECHA_LEVANTE 
    ...
    if($tipo=="XLS" || $tipo=="TXT" || $tipo=="HTML") {
        if(!empty($res)) {
            ...
            //insertar formula en Excel
            $holidays = array();
            $objPHPExcel->getActiveSheet()
            ->setCellValue('G4','=NETWORKDAYS(F5,F4)');
        }  
