<?php
    ...
    $param_fecha="";
    $titulo = "OPERACIONES EN PROCESO";
    

    if($modalidad!="")
        $filtroregimen = "AND ED.TIPO_REGIMEN='$modalidad'";
    else
        $filtroregimen = "";
    ...
    if($numerodo=="" && $numeroped=="" && $ordencompra=="") {
        ...
        $filtros =" WHERE ED.ANULADO='N' $filtroregimen $param_fecha ";
    } else {
    ...
    // Operaciones - Operaciones en Proceso
        $sql = "SELECT ED.CODIGO_SUCURSAL_OP, IM.NOMBRE_IMPORTADOR, ED.NUMERO_PEDIDO, ED.NUMERO_DO, ED.FECHA_APERTURA, MR.NOMBRE_MODALIDAD, LEFT(TRIM(ED.DESCRIPCION),100) PRODUCTO,
        CONCAT(ED.ID_ETAPA_DO, ' ' , ET.NOMBRE_ETAPA) ETAPA , I.FECHA_ESTIMADA_ARRIBO, I.FECHA_MANIFIESTO, I.FECHA_VENCIMIENTO_MCIA, I.FECHA_DOCS_OK, I.FECHA_LEVANTE,
        I.FECHA_DOCS_TRANSPORTADOR, I.FECHA_RETIRO_TOTAL, I.FECHA_ENVIADO_FACTURAR, LEFT(TRIM(ED.ULTIMO_COMENTARIO_CLIENTE),4700) COMENTARIO, LEFT(TRIM(ED.DOCUMENTOS_FALTANTES),250) DOCUMENTOS_FALTANTES,
        E.NOMBRE_PERSONA , E1.NOMBRE_PERSONA
        FROM ESTADOS_DO ED
        LEFT OUTER JOIN MODALIDADES_REGIMENES MR ON MR.ID_MODALIDAD=ED.ID_MODALIDAD
        LEFT OUTER JOIN ETAPAS_DO ET ON ET.ID_ETAPA_DO=ED.ID_ETAPA_DO
        ...
        LEFT OUTER JOIN EMPLEADOS E1 ON ED.ID_EMPLEADO_DIGITA=E1.ID_EMPLEADO
        $filtros  
        $param_fecha
        ";   
    ...
    if($tipo=="XLS" || $tipo=="TXT" || $tipo=="HTML") {
        if(!empty($res)) {
            ...
            $titulo = "Operaciones - Operaciones en Proceso [Estado Operaciones Diarias]";
