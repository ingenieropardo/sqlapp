<?php
    ...
    $sql  = "SELECT IM.NUMERO_DO 'NÚMERO DO',DATE_FORMAT(IM.FECHA_LEVANTE,'%Y') 'AÑO', DATE_FORMAT(IM.FECHA_LEVANTE,'%m') 'MES',
  A.NOMBRE_ADMINISTRACION 'NOMBRE ADMINISTRACIÓN', ED.NUMERO_ORDEN_COMPRA 'NÚMERO ORDEN COMPRA',
  E.NOMBRE_EXPORTADOR 'NOMBRE_EXPORTADOR', CONCAT(CF.CODIGO_CONDICION_ENTREGA,TRIM(CF.LUGAR_ENTREGA)) 'CONDICION ENTREGA',
  LEFT(TRIM(DF.NOMBRE_COMERCIAL),250) 'DESCRIPCION', CF.NUMERO_FACTURA 'NÚMERO FACTURA', CC.NOMBRE_CONTACTO 'UNIDAD SEGUN FACTURA',
  DF.CANTIDAD 'CANTIDAD', IF(CF.CODIGO_CONDICION_ENTREGA='DAP' or CF.CODIGO_CONDICION_ENTREGA='CIP',(
  DF.VALOR_FOB_PRORRATEO/DF.CANTIDAD), (DF.TOTAL_ITEM_USD/DF.CANTIDAD)) 'VALOR UNITARIO 1', IF(CF.CODIGO_CONDICION_ENTREGA='DAP'
  OR CF.CODIGO_CONDICION_ENTREGA='CIP', DF.VALOR_FOB_PRORRATEO, DF.TOTAL_ITEM_USD) 'SUBTOTAL FACTURA', (
  DF.VALOR_FOB_PRORRATEO+DF.EXW_PRORRATEO+DF.SUM_NETO_PRORRATEO) 'VALOR UNITARIO', (COALESCE(DF.EXW_PRORRATEO,0)+COALESCE(
  DF.SUM_NETO_PRORRATEO,0)) 'GASTOS ORIGEN', FD.VALOR_FLETES 'FLETE', FD.VALOR_SEGUROS 'SEGURO',(
  IM.BODEGAJE_GESTION_COMERCIAL_VALOR) 'GTO NACIONALIZACIÓN', FD.TOTAL_ARANCEL 'TOTAL ARANCEL', FD.TOTAL_IVA 'TOTAL IVA',
  IM.BODEGAJE_GESTION_LOGISTICA_VALOR 'BODEGAJE GESTION LOGISTICA VALOR', (COALESCE(DF.TOTAL_ITEM_USD,0) + COALESCE(
  DF.SUM_NETO_PRORRATEO,0) + COALESCE(DF.EXW_PRORRATEO,0) + COALESCE(DF.FLETE_PRORRATEO,0) + COALESCE(DF.SEGURO_PRORRATEO,0))
  'VMCIA GTO', ((COALESCE(DF.TOTAL_ITEM_USD,0)+ COALESCE(DF.SUM_NETO_PRORRATEO,0) + COALESCE(DF.EXW_PRORRATEO,0) + COALESCE(
  DF.FLETE_PRORRATEO,0) + COALESCE( DF.SEGURO_PRORRATEO,0)) * IM.BODEGAJE_GESTION_LOGISTICA_VALOR) 'VMCIA COP',
  FD1.VALOR_FOB 'VALOR FOB', CF.CODIGO_CONDICION_ENTREGA 'CODIGO CONDICION ENTREGA', FD.VALOR_TOTAL_ITEMS 'VALOR TOTAL ITEMS',
  ...
  LEFT OUTER JOIN (SELECT F.NUMERO_DO, SUM(TOTAL_FACTURA_ACTUAL_USD) VALOR_FOB FROM CABEZA_FACTURAS  F  GROUP BY 1 )
    FD1 ON FD1.NUMERO_DO=ED.NUMERO_DO
