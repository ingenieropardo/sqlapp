<?php
    ...
    // Titulo del reporte
    $titulo="REPORTE DAIMLER";
    
    // Nombre del archivo 
    $archivo="reporte_daimler";
    ...
    $dimensionCol = array (10.71,  10.71,  10.71,  16.29,  19.50,  10.71,  10.71,  10.71,  10.71,  10.71,  10.71,  10.71,  10.71,  10.71,  10.71,  10.71,  13.29,  13.29,  13.29,  10.71,  10.71,  10.71,  10.71);
    ...
    $sql  = "SELECT ED.NUMERO_DO 'NUMERO DO', 
        DF.ORDEN_COMPRA 'ORDEN COMPRA', 
        CD.PROFIT_CENTER 'NUMERO VIN', 
        LEFT(CD.INFO_ADICIONAL, 30) 'BANCO', 
        FD.NUMERO_ACEPTACION 'NUMERO ACEPTACION',
        (COALESCE(DF.VALOR_FOB_PRORRATEO,0) + COALESCE(DF.SUM_NETO_PRORRATEO, 0)) 'VALOR FOB',
        DF.FLETE_PRORRATEO 'FLETE PRORRATEO', 
        COALESCE(DF.OTROS_GASTOS_PRORRATEO,0) + COALESCE(DF.DESCARGUE_PRORRATEO,0) 'OTROS GASTOS', 
        DF.SEGURO_PRORRATEO 'SEGURO PRORRATEO',
        COALESCE(DF.VALOR_FOB_PRORRATEO,0) + COALESCE(DF.SUM_NETO_PRORRATEO, 0) +
        COALESCE(DF.FLETE_PRORRATEO,0)     + COALESCE(DF.OTROS_GASTOS_PRORRATEO,0) +
        COALESCE(DF.DESCARGUE_PRORRATEO,0) + COALESCE(DF.SEGURO_PRORRATEO,0) 'VALOR ADUANA', 
        ED.TRM 'TRM',
        ...
        M.VALOR_FOB 'VALOR FOB'
    
        FROM FORMULARIOS_DIS   FD

        LEFT OUTER JOIN DETALLES_FACTURAS               DF   ON DF.ID_FORMULARIO_DI = FD.ID_FORMULARIO_DI
        LEFT OUTER JOIN complemento_detalle_factura     cd   ON DF.ID_DETALLE_FACTURA = CD.ID_DETALLE_FACTURA
        LEFT OUTER JOIN ESTADOS_DO                      ED   ON ED.NUMERO_DO = FD.NUMERO_DO
        LEFT OUTER JOIN IMPORTACIONES                   IM   ON IM.NUMERO_DO = FD.NUMERO_DO
        LEFT OUTER JOIN (SELECT ID_FORMULARIO_DI, 
            SUM(COALESCE(VALOR_FOB_PRORRATEO,0) + COALESCE(SUM_NETO_PRORRATEO,0)) 'VALOR_FOB' 
            FROM DETALLES_FACTURAS GROUP BY 1 )         M   ON M.ID_FORMULARIO_DI=FD.ID_FORMULARIO_DI
